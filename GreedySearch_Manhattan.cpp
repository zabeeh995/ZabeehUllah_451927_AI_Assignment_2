...
void solve(int initial[N][N], int x, int y, int final[N][N])
{
    ...
    root->heuristic = calculateManhattanDistance(initial, final); // Calculate initial heuristic
    pq.push(root);
    ...
    while (!pq.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (WithinLimits(min->x + row[i], min->y + col[i]))
            {
                ...
                child->heuristic = calculateManhattanDistance(child->mat, final);
...
int main()
{
    ...
    cout << "\n\t\t----------------------------------------------------------------------------\n";
    cout << " Enter Initial state of the puzzle in this form \n";
